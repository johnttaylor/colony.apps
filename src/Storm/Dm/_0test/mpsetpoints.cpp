...
/////////////////////////////////////////////////////////////////
namespace {
class Rmw : public MpSetpoints::Client
{
public:
    ///
    int                                         m_callbackCount;
    ...
    ///
    float                                       m_newCoolValue;

public:
    ///
    Rmw() :m_callbackCount( 0 ), m_returnResult( Cpl::Dm::ModelPoint::eNO_CHANGE ), m_newCoolValue( 0.0F ) {}
    ///
    Cpl::Dm::ModelPoint::RmwCallbackResult_T callback( MpSetpoints::Data& data, int8_t validState ) noexcept
    {
        m_callbackCount++;
        if ( m_returnResult != Cpl::Dm::ModelPoint::eNO_CHANGE )
        {
            data.coolSetpt = m_newCoolValue;
        }
...
static MpSetpoints              mp_apple_( modelDb_, info_mp_apple_ );

static Cpl::Dm::StaticInfo      info_mp_orange_( "ORANGE" );
static MpSetpoints              mp_orange_( modelDb_, info_mp_orange_, 75.0F, 70.0F );


////////////////////////////////////////////////////////////////////////////////
TEST_CASE( "MP Setpoints" )
{
    ...
    SECTION( "ReadWrite" )
    {
        ...
        float heat;
        uint16_t            seqNum;
        int8_t              valid = mp_orange_.read( cool, heat );
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
        REQUIRE( Cpl::Math::areFloatsEqual( cool, 75.0F ) );
        REQUIRE( Cpl::Math::areFloatsEqual( heat, 70.0F ) );
        valid = mp_apple_.read( cool, heat, &seqNum );
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
        ...
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
        REQUIRE( Cpl::Math::areFloatsEqual( cool, 77.0F ) );
        REQUIRE( Cpl::Math::areFloatsEqual( heat, 69.0F ) );
        REQUIRE( seqNum + 1 == seqNum2 );

        // Read-Modify-Write with Lock
        Rmw callbackClient;
        callbackClient.m_callbackCount  = 0;
        callbackClient.m_newCoolValue   = 77.5F;
        callbackClient.m_returnResult   = Cpl::Dm::ModelPoint::eCHANGED;
        mp_apple_.readModifyWrite( callbackClient, Cpl::Dm::ModelPoint::eLOCK );
        valid         = mp_apple_.readCool( cool );
        int8_t valid2 = mp_apple_.readHeat( heat );
        REQUIRE( mp_apple_.isNotValid() == false );
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid2 ) == true );
        bool locked = mp_apple_.isLocked();
        REQUIRE( locked == true );
        REQUIRE( Cpl::Math::areFloatsEqual( cool, 77.5F ) );
        REQUIRE( Cpl::Math::areFloatsEqual( heat, 69.0F ) );
        ...
        mp_apple_.writeHeat( OPTION_STORM_DM_MP_SETPOINTS_MIN_HEATING - 1.0F );
        valid = mp_apple_.read( cool, heat );
        printf( "cool=%g, heat=%g\n", cool, heat );
        REQUIRE( mp_apple_.isNotValid() == false );
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
        ...
        // Single writes
        mp_apple_.writeCool( OPTION_STORM_DM_MP_SETPOINTS_MIN_COOLING - 1.0F );
        valid = mp_apple_.read( cool, heat );
        printf( "cool=%g, heat=%g\n", cool, heat );
        REQUIRE( mp_apple_.isNotValid() == false );
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
    ...
    SECTION( "get" )
    {
        ...
        REQUIRE( s == sizeof( MpSetpoints::Data ) );
        s = mp_orange_.getSize();
        REQUIRE( s == sizeof( MpSetpoints::Data ) );

        s = mp_apple_.getExternalSize();
        REQUIRE( s == sizeof( MpSetpoints::Data ) + sizeof( int8_t ) );
...
#define STREAM_BUFFER_SIZE  100
    ...
    SECTION( "export/import" )
    {
        ...
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
        REQUIRE( mp_apple_.isNotValid() == false );
        REQUIRE( Cpl::Math::areFloatsEqual( cool, 70.0F ) );
        REQUIRE( Cpl::Math::areFloatsEqual( heat, 65.0F ) );
        ...
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
        REQUIRE( mp_apple_.isNotValid() == false );
        REQUIRE( Cpl::Math::areFloatsEqual( cool, 70.1F ) );
        REQUIRE( Cpl::Math::areFloatsEqual( heat, 65.1F ) );
    ...
    SECTION( "observer" )
    {
        ...
        AsyncClient<MpSetpoints> viewer1( t1Mbox_, Cpl::System::Thread::getCurrent(), mp_apple_ );
...
#define MAX_STR_LENG    1024
    ...
    SECTION( "toJSON" )
    {
        ...
        SECTION( "Value" )
        {
            ...
            REQUIRE( Cpl::Math::areFloatsEqual( val["cool"], 70.2F ) );
            REQUIRE( Cpl::Math::areFloatsEqual( val["heat"], 65.2F ) );
        }
    ...
    SECTION( "fromJSON" )
    {
        ...
        SECTION( "Write value" )
        {
            const char* json = "{name:\"APPLE\", val:{cool:77.33,heat:66.6}}";
            ...
            REQUIRE( Cpl::Math::areFloatsEqual( cool, 77.33F ) );
            REQUIRE( Cpl::Math::areFloatsEqual( heat, 66.6F ) );
        ...
        SECTION( "Set Invalid" )
        {
            ...
            const char* json = "{name:\"APPLE\", val:{cool:77.8}, invalid:1}";
            bool result = modelDb_.fromJSON( json, &errorMsg, &mp, &seqNum2 );
            CPL_SYSTEM_TRACE_MSG( SECT_, ( "fromSJON errorMsg=[%s])", errorMsg.getString() ) );
            REQUIRE( result == true );
            REQUIRE( seqNum2 == seqNum + 1 );
        ...
        SECTION( "lock..." )
        {
            const char* json = "{name:\"APPLE\", val:{cool:76.4}, locked:true}";
            bool result = modelDb_.fromJSON( json, &errorMsg );
            CPL_SYSTEM_TRACE_MSG( SECT_, ( "fromSJON errorMsg=[%s])", errorMsg.getString() ) );
            REQUIRE( result == true );
            float  cool;
            ...
            REQUIRE( Cpl::Math::areFloatsEqual( cool, 76.4F ) );
            REQUIRE( Cpl::Math::areFloatsEqual( heat, OPTION_STORM_DM_MP_SETPOINTS_MIN_HEATING ) );
            ...
            json   = "{name:\"APPLE\", val:{heat:58.98}, locked:true}";
            result = modelDb_.fromJSON( json, &errorMsg );
            CPL_SYSTEM_TRACE_MSG( SECT_, ( "fromSJON errorMsg=[%s])", errorMsg.getString() ) );
            REQUIRE( result == true );
            REQUIRE( mp_apple_.isLocked() == true );
            ...
            REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
            REQUIRE( Cpl::Math::areFloatsEqual( cool, OPTION_STORM_DM_MP_SETPOINTS_MAX_COOLING ) );
            REQUIRE( Cpl::Math::areFloatsEqual( heat, 58.98F ) );
