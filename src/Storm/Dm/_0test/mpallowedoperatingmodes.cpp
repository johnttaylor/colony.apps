...
/////////////////////////////////////////////////////////////////
namespace {
class Rmw : public MpAllowedOperatingModes::Client
{
public:
    ///
    int                                         m_callbackCount;
    ...
    ///
    Storm::Type::AllowedOperatingModes          m_newValue;

public:
    ///
    Rmw() :m_callbackCount( 0 ), m_returnResult( Cpl::Dm::ModelPoint::eNO_CHANGE ), m_newValue( Storm::Type::AllowedOperatingModes::eCOOLING_AND_HEATING ) {}
    ///
    Cpl::Dm::ModelPoint::RmwCallbackResult_T callback( Storm::Type::AllowedOperatingModes& data, int8_t validState ) noexcept
    {
        m_callbackCount++;
...
static MpAllowedOperatingModes  mp_apple_( modelDb_, info_mp_apple_ );

static Cpl::Dm::StaticInfo      info_mp_orange_( "ORANGE" );
static MpAllowedOperatingModes  mp_orange_( modelDb_, info_mp_orange_ );


////////////////////////////////////////////////////////////////////////////////
TEST_CASE( "MP AllowedOperatingModes" )
{
    ...
    SECTION( "ReadWrite" )
    {
        ...
        uint16_t                           seqNum;
        Storm::Type::AllowedOperatingModes value = Storm::Type::AllowedOperatingModes::eCOOLING_AND_HEATING;
        int8_t                             valid = mp_apple_.read( value, &seqNum );
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == false );

        // Write/Read
        uint16_t seqNum2 = mp_apple_.write( Storm::Type::AllowedOperatingModes::eHEATING_ONLY );
        ...
        // Write
        seqNum = mp_apple_.write( Storm::Type::AllowedOperatingModes::eCOOLING_ONLY );
        valid = mp_apple_.read( value );
        REQUIRE( Cpl::Dm::ModelPoint::IS_VALID( valid ) == true );
        ...
        callbackClient.m_newValue       = Storm::Type::AllowedOperatingModes::eHEATING_ONLY;
    ...
    SECTION( "get" )
    {
        ...
        REQUIRE( s == sizeof( Storm::Type::AllowedOperatingModes ) );
        s = mp_orange_.getSize();
        REQUIRE( s == sizeof( Storm::Type::AllowedOperatingModes ) );

        s = mp_apple_.getExternalSize();
        REQUIRE( s == sizeof( Storm::Type::AllowedOperatingModes ) + sizeof( int8_t ) );
        s = mp_orange_.getExternalSize();
        REQUIRE( s == sizeof( Storm::Type::AllowedOperatingModes ) + sizeof( int8_t ) );

        const char* mpType = mp_apple_.getTypeAsText();
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "typeText: [%s])", mpType ) );
        REQUIRE( strcmp( mpType, "Storm::Dm::MpAllowedOperatingModes" ) == 0 );
    }
...
#define STREAM_BUFFER_SIZE  100
    ...
    SECTION( "export/import" )
    {
        ...
        // Update the MP
        seqNum = mp_apple_.write( Storm::Type::AllowedOperatingModes::eCOOLING_AND_HEATING );
        REQUIRE( seqNum == seqNum2 + 1 );
        ...
        mp_apple_.write( Storm::Type::AllowedOperatingModes::eCOOLING_AND_HEATING );
    ...
    SECTION( "observer" )
    {
        ...
        AsyncClient<MpAllowedOperatingModes> viewer1( t1Mbox_, Cpl::System::Thread::getCurrent(), mp_apple_ );

        // Open, write a value, wait for Viewer to see the change, then close
        mp_apple_.removeLock();
        viewer1.open();
        uint16_t seqNum = mp_apple_.write( Storm::Type::AllowedOperatingModes::eCOOLING_AND_HEATING );
...
#define MAX_STR_LENG    1024
    ...
    SECTION( "toJSON" )
    {
        ...
        SECTION( "Value" )
        {
            uint16_t seqnum = mp_apple_.write( Storm::Type::AllowedOperatingModes::eCOOLING_AND_HEATING, Cpl::Dm::ModelPoint::eUNLOCK );
            mp_apple_.toJSON( string, MAX_STR_LENG, truncated );
            CPL_SYSTEM_TRACE_MSG( SECT_, ( "toJSON: [%s])", string ) );
            ...
            REQUIRE( doc["locked"] == false );
            REQUIRE( doc["invalid"] == 0 );
            REQUIRE( doc["val"] == "eCOOLING_AND_HEATING" );
        }
    ...
    SECTION( "fromJSON" )
    {
        ...
        SECTION( "Write value" )
        {
            const char* json = "{name:\"APPLE\", val:\"eCOOLING_AND_HEATING\"}";
        ...
        SECTION( "Set Invalid" )
        {
            uint16_t seqNum = mp_apple_.write( Storm::Type::AllowedOperatingModes::eHEATING_ONLY );
            const char* json = "{name:\"APPLE\", val:\"eAUTO\", invalid:1}";
            bool result = modelDb_.fromJSON( json, &errorMsg, &mp, &seqNum2 );
            CPL_SYSTEM_TRACE_MSG( SECT_, ( "fromSJON errorMsg=[%s])", errorMsg.getString() ) );
            REQUIRE( result == true );
            REQUIRE( seqNum2 == seqNum + 1 );
        ...
        SECTION( "lock..." )
        {
            const char* json = "{name:\"APPLE\", val:\"eCOOLING_ONLY\", locked:true}";
            bool result = modelDb_.fromJSON( json, &errorMsg );
            CPL_SYSTEM_TRACE_MSG( SECT_, ( "fromSJON errorMsg=[%s])", errorMsg.getString() ) );
            REQUIRE( result == true );
            Storm::Type::AllowedOperatingModes value = Storm::Type::AllowedOperatingModes::eCOOLING_AND_HEATING;
            ...
            json   = "{name:\"APPLE\", val:\"eCOOLING_AND_HEATING\", locked:true}";
            result = modelDb_.fromJSON( json, &errorMsg );
            CPL_SYSTEM_TRACE_MSG( SECT_, ( "fromSJON errorMsg=[%s])", errorMsg.getString() ) );
            REQUIRE( result == true );
            REQUIRE( mp_apple_.isLocked() == true );
            ...
            json   = "{name:\"APPLE\", val:\"eHEATING_ONLY\" }";
            result = modelDb_.fromJSON( json, &errorMsg );
            CPL_SYSTEM_TRACE_MSG( SECT_, ( "fromSJON errorMsg=[%s])", errorMsg.getString() ) );
            REQUIRE( result == true );
            REQUIRE( mp_apple_.isLocked() == true );
